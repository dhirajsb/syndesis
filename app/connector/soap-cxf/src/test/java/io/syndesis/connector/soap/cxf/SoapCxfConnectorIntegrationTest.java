...
public class SoapCxfConnectorIntegrationTest {
    ...
    private static final Connector SOAP_CXF_CONNECTOR = loadConnector();
    ...
    public void shouldInvokeRemoteApis() throws Exception {
        ...
        assertThat(context.createProducerTemplate().requestBody("direct:sayHi",
            REQUEST_PAYLOAD, String.class))
                .isEqualTo(RESPONSE_PAYLOAD);

        wiremock.verify(postRequestedFor(urlEqualTo("/"))
            .withRequestBody(WireMock.equalTo(wrapInEnvelope(REQUEST_PAYLOAD))));
    }
    ...
    private static Connector loadConnector() {
        try (InputStream json = SoapCxfConnectorIntegrationTest.class
            .getResourceAsStream("/META-INF/syndesis/connector/soap-cxf.json")) {
